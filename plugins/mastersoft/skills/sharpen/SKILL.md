...
Give each lens one read-only sub-agent, all in one message so they run in parallel:

```
Agent(subagent_type: "Explore", model: "sonnet", prompt: "Lens: <lens>. Request: <the request in one sentence>. Repo: <root>. Report facts only, each with file:line: <the lens's questions>. No opinions, no plan. Under 250 words.")
```

A lens that one or two Reads answer, you answer yourself. The step is done when every lens has come back with facts or with "nothing found", which is itself a fact for the prompt.
...
- Otherwise stop after the prompt: the user copies it or says to run it.
