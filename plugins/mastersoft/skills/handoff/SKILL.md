...
Produce a small file containing only conversation-only knowledge — what the next chat cannot recover from `git log`, `git diff`, source files, or `CLAUDE.md`. Compute a path of the form `/tmp/handoff-<timestamp>.md` where `<timestamp>` comes from `date +%Y%m%dT%H%M%S` (the only shell call). Use a **literal** `/tmp` path — the `Write` tool does not expand shell variables, so never put `${TMPDIR}` or any `${...}` in the path (it would create a bogus directory). Write directly to that path (the file does not pre-exist, so no prior `Read` is needed — this is what keeps the no-fs-reads contract intact) and print the absolute path on its own line. Do not use `mktemp`: it pre-creates the file, which trips Claude Code's "Read-before-Write on existing files" rule and would force a Read that violates the contract below.
