# Flag Reference (codex-cli 0.156.x)

Scriptable surfaces: `codex exec`, `codex exec review`, `codex exec resume`, `codex exec fork`, `codex cloud …`. Top-level `codex resume` and `codex fork` are interactive pickers. Top-level `codex review` runs non-interactively but lacks `--json`/`-o`/`-m` so the skill cannot monitor it (Hard Rule 1 — use `codex exec review`).
...
| `--strict-config` | | Error out on unknown config.toml fields. |
...
| `--approve-for-me` | | Route approval requests through automatic review in the `workspace-write` sandbox. |
...
`--full-auto` was removed — `codex exec` rejects it (`error: unexpected argument '--full-auto' found`); use `--sandbox workspace-write`. `--ask-for-approval` / `-a` is rejected outright by `codex exec` (`error: unexpected argument '--ask-for-approval' found`); use `-c approval_policy=<untrusted|on-request|never>` instead.
...
| `--ephemeral`, `--skip-git-repo-check`, `--ignore-user-config`, `--ignore-rules`, `--worktree`, `--thread-source`, `--dangerously-bypass-approvals-and-sandbox`, `--dangerously-bypass-hook-trust` | as in `exec`. |

Scope flags `--uncommitted`, `--base`, `--commit` are mutually exclusive — pass exactly one.

`--output-schema` is accepted by `codex exec review` too.

## `codex exec resume` flags

Resume accepts a **subset** of `codex exec` flags (verified against codex-cli 0.156.1):
...
| `--output-schema <FILE>` | JSON Schema constraining final text. |
| `--strict-config` | Error on unknown config.toml fields. |
| `--dangerously-bypass-approvals-and-sandbox` | CI bypass. |
| `--dangerously-bypass-hook-trust` | Run hooks without persisted trust. |
| `--skip-git-repo-check`, `--ephemeral`, `--ignore-user-config`, `--ignore-rules`, `--worktree`, `--thread-source` | as in `exec`. |
| `--json` | JSONL events. |
| `-o, --output-last-message <FILE>` | Final assistant text. |

**NOT accepted by `resume`** (silently dropped or rejected): `--oss`, `--local-provider`, `-p`/`--profile`, `--approve-for-me`, `-s`/`--sandbox`, `-C`/`--cd`, `--add-dir`, `--color`. These come from the persisted session's config; the resumed run cannot re-scope them.
