...
**Cause:** the call reached its Bash timeout (600 000 ms at most, or `BASH_MAX_TIMEOUT_MS`; xhigh frequently exceeds it) and Claude Code moved it to the background, where codex keeps running and spending quota. OR the parent Claude Code session was interrupted/resumed and orphaned the subprocess.
**Solution:** Stop a codex that is still running (`TaskStop` on the task id, or `kill` its PID), then apply Hard Rule 5:
...
**Symptom:** `codex exec` returns nothing. No JSONL events in `$log`. No final in `$final`. At the Bash timeout Claude Code moves it to the background, where it keeps waiting.
...
**Solution:** Append `< /dev/null` to every codex invocation. Closes stdin at exec time. Mandatory on both foreground and background recipes (background is the worst case — the wait loop ends only on the stale-log condition, 5 minutes later).
...
**Solution:** The background wait loop must use three exit conditions, not just terminal-sentinel grep:
```bash
