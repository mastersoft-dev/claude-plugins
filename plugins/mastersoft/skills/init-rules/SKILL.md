...
allowed-tools: Read, Write, Edit, Glob, Grep, Bash(git:*), Bash(mv -n CLAUDE.md AGENTS.md), Bash(mv -n .claude/CLAUDE.md .claude/AGENTS.md), Bash(node ${CLAUDE_PLUGIN_ROOT}/scripts/state.js agents-md-mode), PowerShell, AskUserQuestion, Skill
---
...
3. **Project rules file.** Rules live in `AGENTS.md`, which every coding agent reads; Claude Code 2.1.277+ loads it directly when there is no `CLAUDE.md`, `.claude/CLAUDE.md` or `CLAUDE.local.md` in the repo or a directory above it. Run every command in this step from the repo root.
   ...
   - **No `AGENTS.md` and no team `CLAUDE.md`** (`CLAUDE.md`, `.claude/CLAUDE.md`) → ask once whether to create `AGENTS.md`. On yes:
     ...
     2. Run `mv -n CLAUDE.md AGENTS.md`. If the move is denied, stop and tell the user to rename or delete `CLAUDE.md` themselves, because it would keep `AGENTS.md` from loading.
   ...
   - **Only a team `CLAUDE.md`** → offer to rename it, keeping its directory so relative `@` imports still resolve: `git mv CLAUDE.md AGENTS.md` or `git mv .claude/CLAUDE.md .claude/AGENTS.md`; for an untracked file use `mv -n` with the same paths. Under `claude-md` the rename is safe only together with the `assets/CLAUDE.md.tmpl` pointer in the same directory: offer both as one step, or leave `CLAUDE.md` as it is. On no, leave it and suggest `/mastersoft:refresh-rules` for later.
